...
volatile int buzzerStart = 0;

static uint8_t service1_uuid[]    = {0x71, 0x3D, 0, 0, 0x50, 0x3E, 0x4C, 0x75, 0xBA, 0x94, 0x31, 0x48, 0xF1, 0x8D, 0x94, 0x1E};
static uint8_t service1_chars1[]  = {0x71, 0x3D, 0, 1, 0x50, 0x3E, 0x4C, 0x75, 0xBA, 0x94, 0x31, 0x48, 0xF1, 0x8D, 0x94, 0x1E};
static uint8_t service1_chars2[]  = {0x71, 0x3D, 0, 2, 0x50, 0x3E, 0x4C, 0x75, 0xBA, 0x94, 0x31, 0x48, 0xF1, 0x8D, 0x94, 0x1E};
...
static uint8_t LED_characteristic_is_found = 0;
static uint8_t accel_characteristic_is_found = 0;
// When found the match descriptor, set 1.
static uint8_t descriptor_is_found = 0;
// To save the accel characteristic and descriptor
static DiscoveredCharacteristic            chars_accel;
static DiscoveredCharacteristic            chars_LED;
static DiscoveredCharacteristicDescriptor  desc_of_chars_accel(NULL,GattAttribute::INVALID_HANDLE,GattAttribute::INVALID_HANDLE,UUID::ShortUUIDBytes_t(0));
  ...
  if( NRF_SUCCESS == ble_advdata_parser(BLE_GAP_AD_TYPE_SHORT_LOCAL_NAME, params->advertisingDataLen, (uint8_t *)params->advertisingData, &len, adv_name) ) {
    ...
    if( memcmp("TXRX", adv_name, 4) == 0x00 ) {
      ...
      */
      ...
      cp.minConnectionInterval = 6;
      cp.maxConnectionInterval = 6;
      cp.slaveLatency = 0;
      cp.connectionSupervisionTimeout = 500;
      ble.connect(params->peerAddr, BLEProtocol::AddressType::RANDOM_STATIC, &cp, NULL);
    }
  }
  Serial.println(" ");
}
...
void connectionCallBack( const Gap::ConnectionCallbackParams_t *params ) {
  ...
  Serial.print("The minConnectionInterval : ");
  Serial.println(params->connectionParams->minConnectionInterval);

  Serial.print("The maxConnectionInterval : ");
  Serial.println(params->connectionParams->maxConnectionInterval);

  Serial.print("The Slave Latency : ");
  Serial.println(params->connectionParams->slaveLatency);
...
void disconnectionCallBack(const Gap::DisconnectionCallbackParams_t *params) {
  ...
  accel_characteristic_is_found = 0;
  LED_characteristic_is_found = 0;
...
static void discoveredCharacteristicCallBack(const DiscoveredCharacteristic *chars) {
  ...
  if(chars->getUUID() ==  chars_uuid2) {
    Serial.println("Found Accel characteristic ");
    accel_characteristic_is_found = 1;
    chars_accel = *chars;
  }
  else if (chars->getUUID() == chars_uuid1) {
    Serial.println("Found LED characteristics ");
    LED_characteristic_is_found = 1;
    chars_LED = *chars;
  }
...
static void discoveryTerminationCallBack(Gap::Handle_t connectionHandle) {
  ...
  if(accel_characteristic_is_found == 1) {
    ble.gattClient().discoverCharacteristicDescriptors(chars_accel, discoveredCharsDescriptorCallBack, discoveredDescTerminationCallBack);
  }
}
...
static void discoveredCharsDescriptorCallBack(const CharacteristicDescriptorDiscovery::DiscoveryCallbackParams_t *params) {
  ...
  if(params->descriptor.getUUID().getShortUUID() == 0x2902) {
    ...
    desc_of_chars_accel = params->descriptor;
  }
  Serial.print("connectionHandle       : ");
...
static void discoveredDescTerminationCallBack(const CharacteristicDescriptorDiscovery::TerminationCallbackParams_t *params) {
  ...
  if(descriptor_is_found) {
    ...
    uint16_t value = 0x0001;
    ble.gattClient().write(GattClient::GATT_OP_WRITE_REQ, chars_accel.getConnectionHandle(), desc_of_chars_accel.getAttributeHandle(), 2, (uint8_t *)&value);
  }
}
...
void hvxCallBack(const GattHVXCallbackParams *params) {
  ...
  if(params->data[0] == 1){
    int16_t maxAngle1 = (params->data[1]*256) |  (params->data[2]);
    int16_t strideFreq1 = (params->data[3]*256) |  (params->data[4]);
    double maxAngle = ((double)maxAngle1)/10.0;
    double strideFreq = ((double)strideFreq1)/100.0;
    ...
    int16_t maxAngle1 = (params->data[1]*256) |  (params->data[2]);
    int16_t strideFreq1 = (params->data[3]*256) |  (params->data[4]);
    int16_t maxAngle2 = (params->data[5]*256) |  (params->data[6]);
    int16_t strideFreq2 = (params->data[7]*256) |  (params->data[8]);
    double maxAngleA = ((double)maxAngle1)/10.0;
    double maxAngleB = ((double)maxAngle2)/10.0;
    double strideFreqA = ((double)strideFreq1)/100.0;
    double strideFreqB = ((double)strideFreq2)/100.0;
    Serial.print(maxAngleA);
    Serial.print(", ");
    Serial.println(strideFreqA);
    Serial.print(", ");
    Serial.print(maxAngleB);
    Serial.print(", ");
    Serial.println(strideFreqB);
  ...
  if(params->data[14] == 1){
    ...
    buzzerStart = millis();
//    Serial.print(", ");
//    Serial.println(params->data[14] != 1);
  }
  ...
  */
  /*
  //uint16_t ledVal;
  int16_t Ax = (params->data[0]*256) |  (params->data[1]);
  int16_t Ay = (params->data[2]*256) |  (params->data[3]);
  int16_t Az = (params->data[4]*256) |  (params->data[5]);
  int16_t Gx = (params->data[6]*256) |  (params->data[7]);
  int16_t Gy = (params->data[8]*256) |  (params->data[9]);
  int16_t Gz = (params->data[10]*256) |  (params->data[11]);
  
  Wire.beginTransmission(9);
  for(int i=0;i<12;i++){
    Wire.write(params->data[i]);
  }
  ...
  if (val%20 == 0) {
    ledVal = 1;
    Serial.println("LED toggle1a");
    chars_LED.writeWoResponse(2,(uint8_t *)&ledVal);
    Serial.println("LED toggle1b");
  }
  else if (val%20 == 10) {
    ledVal = 0;
    Serial.println("LED toggle0a");
    chars_LED.writeWoResponse(2,(uint8_t *)&ledVal);
    Serial.println("LED toggle0b");
  }
  uint16_t ledVal;
  uint16_t angle = params->data[0];
  angle += params->data[1]*256;
